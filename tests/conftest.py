...
from pyflashcards.wifi import WifiClient, WifiController, SsidState
from pyflashcards.ble.adapters.bleak_wrapper import BleakWrapperController
from pyflashcards.responses import FlashcardsResp
from pyflashcards.constants import ErrorCode, ProducerType, CmdId
from pyflashcards.communication_client import FlashcardsBle, FlashcardsWifi, FlashcardsResponder
...
from pyflashcards.exceptions import ConnectFailed, FailedToFindDevice
...
class BleCommunicatorTest(FlashcardsBle):
    # pylint: disable=signature-differs
    ...
    def __init__(self, test_version: str) -> None:
        ...
        self._api = api_versions[test_version](self, FlashcardsResponder())
...
class WifiCommunicatorTest(FlashcardsWifi):
    # pylint: disable=signature-differs

    def __init__(self, test_version: str):
        super().__init__(WifiControllerTest())
        self._api = api_versions[test_version](FlashcardsResponder(), self)
...
def _test_parse(self: FlashcardsResp) -> None:
    self._state = FlashcardsResp._State.PARSED
    self._info = [_test_response_id]


class FlashcardsTest(Flashcards):
    def __init__(self, test_version: str) -> None:
        super().__init__(
            target=re.compile("device"),
        ...
        self._api.ble_command.get_open_flashcards_api_version = self._test_return_version
...
@pytest.fixture(scope="module", params=versions)
async def flashcards_client(request):
    original_parse = FlashcardsResp._parse
    FlashcardsResp._parse = _test_parse
    test_client = FlashcardsTest(request.param)
    yield test_client
    FlashcardsResp._parse = original_parse
...
class BleakWrapperTest(FlashcardsBle):
    # pylint: disable=signature-differs
...
async def flashcards_bleak_client(request):
    yield BleakWrapperTest(cameras[request.param])


@pytest.fixture(scope="module", params=list(cameras.keys()))
async def flashcards_ble_no_wifi(request):
    print(f"\nConnecting to a {request.param} for ble tests...")
    g = Flashcards(target=cameras[request.param], enable_wifi=False)
    g.open()
    print("Connected!")
    yield g
    g.close()
...
@pytest.fixture(scope="module", params=list(cameras.keys()))
async def flashcards_ble_and_wifi(request):
    print(f"\nConnecting to a {request.param} for wifi tests...")
    g = Flashcards(target=cameras[request.param])
    g.open()
    print("Connected!")
    yield g
    g.close()
