...
"""Implements top level interface to Flashcards module."""
...
from pyflashcards.exceptions import InvalidOpenFlashcardsVersion, ResponseTimeout, InvalidConfiguration
from pyflashcards.ble import BLEController, UUID, BleDevice
from pyflashcards.ble.adapters import BleakWrapperController
from pyflashcards.wifi import WifiController
from pyflashcards.wifi.adapters import Wireless
from pyflashcards.util import SnapshotQueue
from pyflashcards.responses import FlashcardsResp
from pyflashcards.constants import CmdId, ErrorCode, StatusId, QueryCmdId, ProducerType
from pyflashcards.api import (
    Api,
    api_versions,
    BleCommands,
    BleSettings,
...
from pyflashcards.communication_client import FlashcardsBle, FlashcardsWifi
...
def _ensure_initialized_acquire_ready_semaphore(wrapped, instance, args, kwargs) -> Callable:  # type: ignore
    ...
    Raises:
        FlashcardsNotInitialized: The function can't be used yet because the Flashcards device isn't initialized
...
class Flashcards(FlashcardsBle, FlashcardsWifi, Generic[BleDevice]):
    """The top-level BLE and Wifi interface to a Flashcards device.

    See `Flashcards <https://flashcards.github.io/OpenFlashcards/python_sdk>`_ for complete documentation.
    ...
    - discovering Flashcards version
    - transferring data
    ...
    If no target arg is passed in, the first discovered Flashcards device will be connected to.
    ...
    Args:
        identifier (Pattern, optional): Last 4 of camera name / serial number (i.e. 0456 for Flashcards0456). Defaults
            to None (i.e. connect to first discovered Flashcards)
    ...
    _base_url = "http://10.5.5.9:8080/"  #: Hard-coded Flashcards base URL

    class _InternalState(enum.IntFlag):
        """State used to manage whether the Flashcards instance is ready or not."""
    ...
    def __init__(
        ...
        # Initialize Flashcards Communication Client
        FlashcardsBle.__init__(self, ble_adapter(), self._disconnect_handler, self._notification_handler, target)
        FlashcardsWifi.__init__(self, wifi_adapter())
        ...
        self._active_resp: Dict[UUID, FlashcardsResp] = {}
        ...
        self._out_q: "Queue[FlashcardsResp]" = Queue()
        ...
        if self._maintain_ble:
            ...
            self._internal_state = Flashcards._InternalState.ENCODING | Flashcards._InternalState.SYSTEM_BUSY
            self._state_thread = threading.Thread(target=self._maintain_state, name="state", daemon=True)

    def __enter__(self) -> "Flashcards":  # pylint: disable=missing-return-doc
        self.open()
        return self
    ...
    def is_ble_connected(self) -> bool:
        """Are we connected via BLE to the Flashcards device?
    ...
    def is_wifi_connected(self) -> bool:
        """Are we connected via Wifi to the Flashcards device?
    ...
    def is_encoding(self) -> bool:
        ...
        return self._internal_state & Flashcards._InternalState.ENCODING == 1
    ...
    def is_busy(self) -> bool:
        ...
        return self._internal_state & Flashcards._InternalState.READY == 0
    ...
    def open(self, timeout: int = 10, retries: int = 5) -> None:
        ...
        start maintenance, and get Flashcards version..
        ...
        version = self.ble_command.get_open_flashcards_api_version().flatten
        version_str = f"{version.major}.{version.minor}"
        try:
            self._api = api_versions[version_str](self, self)
        except KeyError as e:
            raise InvalidOpenFlashcardsVersion(version_str) from e
        logger.info(f"Using Flashcards API version {version_str}")
    ...
    def close(self) -> None:
        """Safely stop the Flashcards instance.
    ...
    def get_update(self, timeout: float = None) -> FlashcardsResp:
        ...
        Returns:
            FlashcardsResp: Received notification
        """
    ...
    def keep_alive(self) -> bool:
        ...
        This is sent automatically by the Flashcards instance if its `maintain_ble` argument is not False.
    ...
    def _notification_handler(self, handle: int, data: bytearray) -> None:
        ...
        try:
            ...
            # It's possible a non-Flashcards service (i.e. battery) sends a notification before we have
        ...
        if uuid not in self._active_resp:
            self._active_resp[uuid] = FlashcardsResp(self._parser_map, info=[uuid])
            ...
            if (
                ...
                with self._state_condition:
                    if response[StatusId.ENCODING] is True:
                        self._internal_state |= Flashcards._InternalState.ENCODING
                    else:
                        self._internal_state &= ~Flashcards._InternalState.ENCODING
            ...
            if (
                ...
                with self._state_condition:
                    if response[StatusId.SYSTEM_READY] is True:
                        self._internal_state &= ~Flashcards._InternalState.SYSTEM_BUSY
                    else:
                        self._internal_state |= Flashcards._InternalState.SYSTEM_BUSY
                    self._state_condition.notify()
    ...
    def _write_characteristic_receive_notification(self, uuid: UUID, data: bytearray) -> FlashcardsResp:
        ...
        Returns:
            FlashcardsResp: parsed notification response data
        """
        ...
        if (
            ...
            and not (
                FlashcardsResp._from_write_command(self._parser_map, uuid, data).id is CmdId.SET_SHUTTER
                and data[-1] == 0
            )
        ):
            logger.debug(
                f"{FlashcardsResp._from_write_command(self._parser_map, uuid, data).id} acquiring semaphore"
            )
            self._ready.acquire()
            logger.debug(f"{FlashcardsResp._from_write_command(self._parser_map, uuid, data).id} has semaphore")
            have_semaphore = True

        # Store information on the response we are expecting
        self._sync_resp_wait_q.put(FlashcardsResp._from_write_command(self._parser_map, uuid, data))
        # Perform write
        ...
        if self._maintain_ble:
            ...
            if response.cmd is CmdId.SET_SHUTTER and data[-1] == 1:
                while not self._internal_state & Flashcards._InternalState.ENCODING:
            ...
            if have_semaphore:
                self._ready.release()
                logger.debug(
                    f"{FlashcardsResp._from_write_command(self._parser_map, uuid, data).id} released the semaphore"
                )

        return response

    def _read_characteristic(self, uuid: UUID) -> FlashcardsResp:
        ...
        return FlashcardsResp._from_read_response(self._parser_map, uuid, received_data)

    def _open_wifi(self, timeout: int = 15, retries: int = 5) -> None:
        """Connect to a Flashcards device via Wifi.
    ...
    def _get(self, url: str) -> FlashcardsResp:
        """Send an HTTP GET request to an Flashcards endpoint.
        ...
        Returns:
            FlashcardsResp: response
        """
        url = Flashcards._base_url + url
        ...
        response = FlashcardsResp._from_http_response(self._parser_map, request)
        return response

    @_ensure_initialized_acquire_ready_semaphore
    def _stream_to_file(self, url: str, file: Path) -> None:
        """Send an HTTP GET request to an Flashcards endpoint to download a binary file.
